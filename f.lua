...
local function apiPost(endpoint, body, silent)
    ...
    if not ok_enc then
        if not silent then log("[ ERR ] JSON encode that bai") end
        return nil
    end
    ...
    if not ok then
        if not silent then log("[ ERR ] Request loi: " .. tostring(res)) end
        return nil
    end
...
local function verify(key, tabId)
    ...
    if not data.success then
        ...
        elseif code == "banned"       or err:find("ban")        then
        ...
        elseif code == "not_found"    or err:find("not found")  then
            kick("Key Khong Ton Tai!")
        elseif code == "not_redeemed" or err:find("not redeem") then
            kick("Key Chua Kich Hoat!\nDung /redeem trong Discord.")
        else
            kick("Key Khong Hop Le!\nCode: " .. tostring(data.code or "unknown"))
        end
        return false
    end
